...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${cityName} Veteriner | Veterineri Bul`,
    description: `${cityName} ilinde doğrulanmış veterinerler. Online randevu, uygun fiyatlar ve güvenilir hizmet. Veterineri Bul ile ${cityName} veteriner bulun.`,
    alternates: { canonical: `/${citySlug}-veteriner` },
  };
}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
              {vets.map((vet) => (
                <div key={vet.id} className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow">
                  ...
                  <Link href="/auth/register">
                    <Button size="sm" className="w-full bg-[#F97316] hover:bg-[#EA6A0A] text-white text-xs">
